...
\subsection{Formula for quotient}
\begin{corollary}[Residue at a simple pole of a quotient]
  Let $p, q$ be holomorphic in a neighborhood of $z_0$. If
  \[
  q(z_0) = 0 \qquad \text{and} \qquad q'(z_0) \neq 0,
  \]
  then $f = \dfrac{p}{q}$ has a simple pole at $z_0$ and
  \[
  Res_{z_0}\!\left(\frac{p}{q}\right) = \frac{p(z_0)}{q'(z_0)}.
  \]
\end{corollary}

\begin{remark}
  Indeed, since $z_0$ is a simple zero of $q$, we can write $q(z) = (z-z_0)\,\tilde{q}(z)$ with $\tilde{q}(z_0) = q'(z_0) \neq 0$. Hence
  \[
  Res_{z_0}\!\left(\frac{p}{q}\right)
  = \lim_{z \to z_0} (z-z_0)\,\frac{p(z)}{q(z)}
  = \lim_{z \to z_0} \frac{p(z)}{\tilde{q}(z)}
  = \frac{p(z_0)}{q'(z_0)}.
  \]
\end{remark}
