...
\\ 
To minimize \textbf{non-convex} function the same algorithm can be used. However, different initizialisations yield to different results because there is more than one point with derivative equal to zero.
...
\]
\[
  = - \sum_{i \in positives samples} ln \sigma(w^Tx_i) - \sum_{i \in negative samples} ln(1 - \sigma(w^Tx_i))
\]
