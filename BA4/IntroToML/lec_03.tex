...
\begin{definition}[Loss function]
  The loss function (general ML concept) : $\mathcal{L}(\hat{y_i}, yi)$ computes an error value between the prediction and the true value.
\end{definition}
In general we use the empirical risk (the mean of the loss on the training or test data) \[
  \hat{R}(w) = \frac{1}{N_t}\sum_{i=1}^{N_t}\mathcal{L}(y_i, \hat{y}_i) 
\] with a different loss function $\mathcal{L}(y_i, \hat{y}_i)$.
...
To learn the parameters $w$ need a loss function that works well with probabilities, and keeps the problem convex: \\
\textbf{Cross-entropy} :

\textbf{1D case}
\[
  R(w) = - \sum_{i = 1}^N  (y_i \ln(\hat{y_i}) + (1-y_i) \ln(1 - \hat{y_i}))
= - \sum_{i \in positive samples} \ln(\hat{y_i}) - \sum_{i \in negative samples} \ln(1 - \hat{y_i})
\] as $y_i$ is either 0 or 1 \\
...
As in practice we have more than one parameter (i.e \textbf{w} $\in \mathbb{R}^{D+1}$), we use the function gradient: \[
  ...
  \in \mathbb{R}^D
\]
\textbf{Recap }  The gradient at a point $w$ has the direction of the greatest increase of the function at $w$ and its magnitude is the rate of increase in that direction. \\
...
\textbf{Cross-Entropy (multiple outputs)}: \[
  R(w) = - \sum_{i = 1}^N y_i \ln \sigma(w^Tx_i) + (1 - y_i) \ln(1 - \sigma(w^Tx_i)) 
\]
\[
  = - \sum_{i \in positive samples} \ln \sigma(w^Tx_i) - \sum_{i \in negative samples} \ln(1 - \sigma(w^Tx_i))
\]
To minimize it via gradient descent we compute its gradient w.r.t $w$ \[
