...
\] , where the extra dimension contains a 1 to account for $w^{\left(0\right)}$ (bias).\\
...
\]
This is a convex function of $w$, which means that we search for parameters \textbf{$w^*$} s.t
\[
 \nabla_w R(w^*) = 0 
\]   
...
\]
$(D+1) \times (D+1)$ matrix $\cdot $ $(D+1) \times 1$ vector = $(D+1) \times 1$ vector
...
\[
  \text{MSE} = \frac{1}{N_t}\sum_{i=1}^{N_t}(\hat{y_i} - y_i)^2 
\]
...
\[
  R(W) = \min_w \sum_{i=1}^{N}\left\|W^Tx_i - y_i\right\|^2
\]
where $\left\|a\right\| = \sqrt{\sum_{i=1}^{C}(a^{(i)})^2}$ indicates the norm of a vector (here $a \in \mathbb{R}^C$), it's the equivalent of the least square but for vectors.\\
...
\begin{itemize}
  \item Given $N$ pairs {$(x_i, y_i)$}, we can use the least-square loss to write
    \[
      R(w) = \min_w \sum_{i=1}^{N}(x_i^Tw - y_i)^2  
    \]
\end{itemize}
...
However, this model isn't that good because with some data, the \textbf{decision boundary} (threshold) will not react well and misclassified some samples.  \\
  \includegraphics[width=0.5\linewidth]{images/img_20260304_185637.png}
  \includegraphics[width=0.5\linewidth]{images/img_20260304_192312.png}
So we will introduce an other model the \textbf{logistic regression}.
\clearpage
