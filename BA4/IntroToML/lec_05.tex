...
We assign the label/value of the most common nearest neighbor. \\
...
\begin{enumerate}
  \item Compute the distance between the test sample $x$ and all training samples \textbf{\{$x_i$\}}
...
\item The features have to be \textbf{normalized} to compute the distance (all features to the same weight).
...
\item \textbf{Curse of dimensionality}, in the real world data tends to be very high dimensional (e.g., for an image of 536 x 356 in 3D $x_i \in \mathbb{R}^{536 \cdot 356 \cdot 3}$), in such dimension, all points tend to be far apart. 
\end{itemize}
...
For class that need a polynomial of degree bigger than 2, what we can do, is to look 
where we want our gradient to vanish after the application of $\phi(\cdot)$. Then we start with 
$(\phi(x))'$ and integrate it to find the appropriate $\phi(x)$. 

\textbf{Example} if our gradient vanishes at x = -2, 0, 2 
\[
  (\phi(x))' = (x+2)(x-0)(x-2) = x(x^2 - 4) = x^3 - 4x \implies \phi(x) = \int \phi(x)' dx 
\]
...
This new function is still convex, and its gradient is given by: \[
  \nabla R = 2 \sum_{i=1}^{N}\phi(x_i)(\phi(x_i)^T w - y_i) 
\] 
