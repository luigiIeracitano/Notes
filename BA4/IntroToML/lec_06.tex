...
When using non linear model we should care about the model complexity, because we by using those type of model
 we have had flexibility, that can make the model too complex fot the task at hand. 
...
the piecewise linear function will probably work poorly on it (it is an overfitted model : model that go though each point of the training set but that is completly wrong between those points).
\newpage
...
\begin{itemize}
  ...
  \item Find the best values for one hyper-parameter (e.g, $k$ in kNN), for this, one treats each value of the hyper-parameter as a spearate model.
\end{itemize}
