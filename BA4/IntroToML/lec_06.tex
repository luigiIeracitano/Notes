...
E.g,: a high degree polynomial is more complex than a low degree one.
...
\begin{enumerate}
  \item Randomly split the dataset into $k$ partitions (e.g., $k = 3$, shown with different colors)
...
\]
where $R(w)$ is the empirical risk, $E_w(w)$ the regularizer, $\lambda$ is a hyper-parameter that defines the influence of the regularizer ($\lambda > 0$). 

So know we want not only to minimize the error on the data but to keep the weight as low as possible too.
By adding this constraints, it reduces the complexity of overfitted model by making them simpler 
(the higher the hyperparameter $\lambda$, the stronger is the constraint, e.g., $\lambda = 1'000'000$ force the weights to be almost zero and results on a flat line ($\hat{y} \approx 0$)).

\subsubsection{Example of regularizer}
\textbf{L1 regularizer : } sum of the absolute values \[
  E_w(w) = \sum_{j} \left|w^{(j)}\right|
\]
\textbf{L2 regularizer : } sum of squares
of the weights\[
  E_w(w) = w^Tw
\]

\subsubsection{Regularized linear regression}
We use the L2 regularizer, this lets us write regularized linear regression as \[
...
In fact, some implementation of the logistic regression has a regularizer by default (e.g., scikit-learn).

\subsubsection{Regularization and SVM}
As the SVM training problem is expressed as \[
