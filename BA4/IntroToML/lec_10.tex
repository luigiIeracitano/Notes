...
\paragraph{Example} with $X$ of size $3 \times 3 \times 1$ and kernel $K$ of size $2 \times 2 \times 1$, the output is of size $(3-2+1) \times (3-2+1) = 2 \times 2$:
\[
...
\paragraph{Example} with 3 filters of size $5 \times 5$ on a $28 \times 28$ input, we obtain a 
...
\textbf{Example } $C \times 3 \times 5$ input with a padding of $(2,1)$ becomes $C \times 7 \times 7$ (2 lines of zeroes (zero-padding) up and down, and 1 column of zeroes to the left and to the right). 
\begin{center}
...
\textbf{Example } stride of $(2,2)$ on a padded input with a $3 \times 3$ kernel: the kernel only 
...
Eventually, one typically wants to output:
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Batch normalization} : normalizes across the batch (per channel).
  \item \textbf{Layer normalization} : normalizes across all channels (per sample).
  \item \textbf{Instance normalization} : normalizes per channel and per sample.
  \item \textbf{Group normalization} : normalizes within groups of channels.
\end{itemize}
...
networks (FCNs)} like \textbf{U-Net}, widely used for 
...
\begin{itemize}
  \item \textbf{Big data}: large annotated datasets, e.g., ImageNet with millions of labeled images.
  \item \textbf{GPUs}: massively parallel computations make training of deep networks feasible.
\end{itemize}

\subsubsection{Tricks of the trade}
\begin{itemize}
  \item \textbf{Pre-training}: people often start with a standard architecture pre-trained on ImageNet, then fine-tune (adapt) it on the target task. This transfers learned features and accelerates training.
