...
\section{Recall from Linear Algebra}
First let's recall some important notions from linear algebra. 

\textbf{Rank:} number of lines (or columns) lineary independent. Intuitively: how many "truly different" directions are encoded in the matrix. 
So for a matrix $\mathbb{R}^{N \times D}$, the rank is at most $\min(N,D)$.

\textbf{Eigenvector and eigenvalues:} \[
  Av = \lambda v
\]with $v$ an eigenvector and $\lambda$ the associate eigenvalue.

Geometrical intuition: an eigenvector is a direction that remains the same after the application of the transformation $A$, it is only stretched or compressed by a factor $\lambda$. 
...
\]
The left-hand side is exactly the variance of the projected data ($\lambda_i$ = variance in the direction of $w_{(i)}$). Since we want to
...
For $d > 1$ output dimensions, we proceed recursively:
\begin{itemize}
...
(symmetric) matrix are orthogonal and of unit norm.  The projected data $y_i = W^T(x_i - \bar{x}) \in \mathbb{R}^d$
has covariance matrix
\[
\frac{1}{N}\sum_{i=1}^N y_i y_i^T = W^T C W = W^T W \Lambda W^T W = \Lambda,
\]
where $\Lambda \in \mathbb{R}^{d \times d}$ is diagonal with $\Lambda_{ii} = \lambda_i$ for $i \leq d$.
Since $\Lambda$ is diagonal, the projected dimensions are uncorrelated.
...
\begin{itemize}
  ...
  \item \textbf{Keep all eigenvectors with non-zero eigenvalues}: (remove direction that gives no information (variance)),
  the resulting $\{y_i\}$ have $d = rank(C) \leq D$ with no loss of information. If we have $N < D$ then $d = rank(C) \leq \min(N-1, D)$. ($N - 1$ because we lose one degree of liberty to center the data).
...
\begin{itemize}
  ...
  \item \textbf{Concentric spheres in 3D} (one inner ``ball'' of class 2 surrounded by an outer ``shell'' of class 1):
    \begin{center}
       \includegraphics[width=0.7\linewidth]{images/img_20260505_202034.png}
\includegraphics[width=0.7\linewidth]{images/img_20260505_202128.png}
    \end{center}
