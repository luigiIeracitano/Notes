...
The mesure of closeness/error (loss function) between $y_i$ and $\hat{y_i}$ is represented by the Euclidean distance: \\ 
...
Training can then be expressed as the least-squared minimization problem:
\[ R(w) = min_{w^{\left(0\right)}, w^{\left(1\right)}} \frac{1}{N} \sum_{i= 1}^{N} d^2{\left(\hat{y_i},y_i\right)} \]
