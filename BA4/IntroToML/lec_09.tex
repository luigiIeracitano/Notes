...
jointly with the classifier or with the linear model that comes afterward (same for regression)}, removing the need for hand-crafted features. 
...
Thus we learn jointly all the weights of the network in a single training pass: the weights of the hidden layers define the representation of the data, while the weights of the last layer correspond to the final linear model.
...
\textbf{classification} (softmax + cross-entropy). 

This framework is referred to as \textit{neural network}, \textit{multilayer perceptron (MLP)}, 
or \textit{deep learning} when there are at least two hidden layers.

\subsection{Parametric feature expansion}
As we have seen, parametric feature expansion isn't optimal as we need to define manually the polynomial expansion $\phi(x)$ and we would like to learn the representation conjointly with the model. 
...
\begin{enumerate}
  \item \textbf{Forward pass :} propagate $x_i$ through each layer to compute and store $z_{(l)}$ 
...
\]
This is expensive for large $N$ as at each iteration we have to compute the gradient over all the samples of the dataset. 
...
\paragraph{Adaptive learning rate} Methods such as \textbf{ADAM} maintain a moving average per coordinate 
...
\begin{itemize}
  ...
  \item \textbf{Dropout}: randomly remove 
