...
a_{(1)}^{(M)}
  \end{pmatrix}
\]
Then we apply $f$ elementwise to $a_{(1)}$, and it gives our complete $M-$dimensional \textit{hidden represantation} \[
  x \to \begin{pmatrix}
    f(a_{(1)}^{(1)})   \\
  f(a_{(1)}^{(2)}) \\
    \vdots \\
...
\]
As we have seen with the $1$D example, because we use a matrix-vector product at each layer, the layers are called 
...
\]
This is expensive for large $N$. \textbf{Stochastic gradient descent (SGD)} 
