...
Single number that combines precision and recall  \[
  ...
  \]
\end{definition}
The F1 score is used as it provides a balanced measure between precision and recall, which is particularly useful when the classes are imbalanced. 
\paragraph{Example}
Class A: 990 samples, Class B: 10 samples. With an accuracy of 99\% we don't know if our model recognizes the class B or it predict every samples as class A. But with F1 score this would be detected (poor F1 score).
...
A good model as a plot near the top left corner (high TP rate and low FP rate) as we mesure the quality of the model with the Area under the curve \textbf{(AUC)}(goes from 0 to 1), 1.0 that the model is perfect, 0.5 is the result if the model makes random predictions, and less is worse than randomness. 
...
The decision boundary between two classes $k$ and $j$ is defined as the set of points $x$ such that \[
...
\textbf{Example} in a 5-class problem, a sample in class 2 is represented as:
\[
  y = \begin{pmatrix} 0 \\ 1 \\ 0 \\ 0 \\ 0
  \end{pmatrix}
\] This is referred as \textbf{one-hot-encoding}.  \\ 
As this is similar to making the model output multiple values (as for multi-output regression), we therefore again use a matrix \textbf{W} $\in \mathbb{R}^{\left(D+1\right)\times C}$, s.t \[
...
\paragraph{Example} of success and failure case of 2D inputs and 3 classes least-square classification:
\begin{center}
...
As in the binary case, the multi-class least-square classification suffer from outliers, because it tries to minimize the distance (regression). So we need an other model like the logistic regression one but for multi-class: \textbf{Multi-class logistic regression} WOWOWOW !!
...
\](i \leftarrow  go through the samples, k \leftarrow  go through the classes) 
\\
...
At test time, given a new input sample $x$, the probability for any class $k$ is computed via the softmax function as \[
  \hat{y}^{\left(k\right)}(x) = \frac{\exp((w_{\left(k\right)}^*)^Tx)}{\sum_{j=1}^{C}exp((w_{\left(j\right)}^*)^Tx) } 
\]
...
Instead one can use several binary classifier (e.g, one-vs-rest or one-vs-one, however leaves ambiguities and inconsistencies as a sample can be classified being from more that one class at a time).
...
\end{center}
Unfortunately, no linear model can directly fit non linear data, so we will go from linear to non linear models.
