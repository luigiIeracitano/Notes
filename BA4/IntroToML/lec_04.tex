...
\end{wrapfigure}
For binatry case we could use confusion matrix:\\
...
The decision boundary between two classes $k$ and $j$ is defined as the set of points \\\textbf{x} such that $\hat{y}^{\left(k\right)}(x) = \hat{y}^{\left(j\right)}(x)$.\\
...
\textbf{e.g.} in a 5-class problem, a sampe in class 2 is represented as:
\[
...
\]
where each $y_i$ is now a $C-$dimensional vector with a single one at the inex corresponding to the class label and zeroes everywhere else.
...
\]
$W$ = weight matrix (i.e $W = [W_0, W_1]$, \quad $W_0, W_1 _$ weight classes 0 and 1)
...
But now, the probability for a class $k$ is given by the \textbf{softmax} function (takes scores and transform them into probabilities) \[
...
The empirical risk can then be derived from the multi-class version of the cross entropy, which yields \[
  R(W) = - \sum_{i=1}^{N}\sum_{k=1}^{C}y_i^{\left(k\right)}\ln \hat{y}_i^{\left(k\right)}  
...
At test time, given a new input sample \textbf{x}, the probability/score for any class $k$ is computed via the softmax function as \[
  \hat{y}^{\left(k\right)}(x) = \frac{\exp((w_{\left(k\right)}^*)^Tx)}{\sum_{j=1}^{C}exp((w_{\left(j\right)}^*)^Tx) } 
\]
