...
// sinon memcpy(ptr, données, taille) ou c'est pas juste une valeur qui est copié mais un bloc de donnée, par exemple si on met un tableau alors chaque élément est copié à la position correspondante 
\end{lstlisting}
...
vector* vector_enlarge(vector* v) {
  if (v != NULL) {
    ...
    type_el* temp = realloc(result.content, result.allocated * sizeof(type_el));
    if (temp == NULL) return NULL; // v inchangé
    
    result.content = temp; // si succès de réallocation on met à jour le pointeur
